...
const STATUS_EM_ANDAMENTO = [
  "aguardando_feedback",
  "aguardando_aceite",
  "validada", // legado durante rollout
  "aguardando_analise", // legado durante rollout
];
...
const STATUS_PENDENTE = ["aberta", ...STATUS_EM_ANDAMENTO];
...
export default function HomePage() {
  ...
  const contagemPorAba = useMemo(() => {
    ...
    for (const aba of ABAS_FILTRO) {
      ...
      contagem[aba.chave] = ncs.filter((nc) => statusAlvo.includes(nc.status)).length;
    }
    return contagem;
  }, [ncs]);
  ...
  const ncsPendentes = ncs.filter((nc) => STATUS_PENDENTE.includes(nc.status)).length;
    ...
    <div>
      ...
      <Container className="sg-container">
        <CabecalhoPagina
          ...
          acoes={
            <Link to="/abrir-nc" className="sg-btn sg-btn--primario">+ Abrir NC</Link>
          }
        />
          ...
          <>
            <div className="row g-3 mb-4">
              <div className="col-sm-6 col-lg-3">
                <CardMetrica rotulo="Total de NCs" valor={totalNcs} descricao="Não conformidades visíveis" cor="azul" />
              </div>
              <div className="col-sm-6 col-lg-3">
                <CardMetrica rotulo="Abertas" valor={ncsAbertas} descricao="Aguardando avaliação" cor="amarela" />
              </div>
              <div className="col-sm-6 col-lg-3">
                <CardMetrica rotulo="Pendentes" valor={ncsPendentes} descricao="Backlog ativo" cor="laranja" />
              </div>
              <div className="col-sm-6 col-lg-3">
                <CardMetrica rotulo="Resolvidas" valor={ncsResolvidas} descricao="Concluídas" cor="verde" />
              </div>
            </div>
            ...
            <div className="sg-acoes-rapidas mb-5">
              ...
              <Link to="/" className="sg-acao-rapida" onClick={(e) => { e.preventDefault(); atualizar(); }}>
                <span className="sg-acao-rapida__icone" aria-hidden="true">↻</span>
            ...
            {ncsRecentes.length === 0 ? (
              <EstadoVazio titulo="Sem atividades recentes" descricao="As últimas não conformidades aparecerão aqui." />
            ) : (
                ...
                <div className="d-flex flex-column">
                  {ncsRecentes.map((nc, indice) => (
                    <Link
                      ...
                      style={{ borderBottom: indice < ncsRecentes.length - 1 ? "1px solid var(--borda)" : "none" }}
                    >
                      <div className="d-flex align-items-center gap-3 min-w-0">
                        <span className="sg-nc-card__numero">NC #{nc.id}</span>
                        <div className="min-w-0">
                          <div className="fw-semibold text-truncate">{nc.colaborador || "-"}</div>
                          <div className="texto-xs texto-suave">{formatarData(nc.data)} · {nc.criticidade || "-"}</div>
                        </div>
                      </div>
              ...
              {ABAS_FILTRO.map((aba) => (
                <Nav.Item key={aba.chave}>
                  <Nav.Link eventKey={aba.chave}>
                    {aba.rotulo}
                    <span className="texto-xs texto-suave ms-1">({contagemPorAba[aba.chave] ?? 0})</span>
                  </Nav.Link>
                </Nav.Item>
              ))}
            </Nav>
            ...
            {ncsFiltradas.length === 0 ? (
              <EstadoVazio titulo="Nenhuma Não Conformidade encontrada" descricao="Não há registros para este filtro." />
            ) : (
