  ...
  return (
    <div>
      ...
      <Container className="sg-container" style={{ maxWidth: "900px" }}>
        ...
        <Link to="/" className="sg-voltar mb-3 d-inline-flex">&larr; Voltar para a lista</Link>
          ...
          <div className="d-flex flex-column gap-3">
            <div className="sg-card">
              <div className="sg-card-body p-4">
                ...
                <dl className="mb-0">
                  <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Ocorrência</dt><dd className="sg-detalhe__valor">{formatarData(nc.data)}</dd></div>
                  <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Colaborador analisado</dt><dd className="sg-detalhe__valor">{nc.colaborador || "-"}</dd></div>
                  <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Setor</dt><dd className="sg-detalhe__valor">{nc.setor || "-"}</dd></div>
                  <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Chamado</dt><dd className="sg-detalhe__valor">{nc.chamado || "-"}</dd></div>
                  <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Reincidência</dt><dd className="sg-detalhe__valor">{nc.reincidencia ?? "-"}</dd></div>
                  <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Descrição</dt><dd className="sg-detalhe__valor">{nc.descricao || "-"}</dd></div>
                  <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Causas</dt><dd className="sg-detalhe__valor">{nc.causas?.length > 0 ? nc.causas.join(", ") : "-"}</dd></div>

                  {podeVerDetalhesCompletos && nc.motivo_invalidacao && (
                    <div className="sg-detalhe">
                      <dt className="sg-detalhe__rotulo" style={{ color: "var(--erro)" }}>Motivo da invalidação</dt>
                      <dd className="sg-detalhe__valor" style={{ color: "var(--erro)" }}>{nc.motivo_invalidacao}</dd>
                    </div>
                  )}
                  {podeVerDetalhesCompletos && nc.feedback && (
                    <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Feedback</dt><dd className="sg-detalhe__valor">{nc.feedback}</dd></div>
                  )}
                  {podeVerDetalhesCompletos && nc.texto_aceite && (
                    <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Aceite registrado</dt><dd className="sg-detalhe__valor fst-italic">"{nc.texto_aceite}"</dd></div>
                  )}
                  {podeVerDetalhesCompletos && (
                    <>
                      {nc.validado_em && <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Validado em</dt><dd className="sg-detalhe__valor">{formatarDataHora(nc.validado_em)}</dd></div>}
                      {nc.feedback_aplicado_em && <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Feedback aplicado em</dt><dd className="sg-detalhe__valor">{formatarDataHora(nc.feedback_aplicado_em)}</dd></div>}
                      {nc.aceito_em && <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Aceito em</dt><dd className="sg-detalhe__valor">{formatarDataHora(nc.aceito_em)}</dd></div>}
                      <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Criado em</dt><dd className="sg-detalhe__valor">{formatarDataHora(nc.criado_em)}</dd></div>
                      <div className="sg-detalhe"><dt className="sg-detalhe__rotulo">Atualizado em</dt><dd className="sg-detalhe__valor">{formatarDataHora(nc.atualizado_em)}</dd></div>
                    </>
                  )}
                </dl>
                ...
                {podeVerResumo && (
                  <div className="sg-alerta sg-alerta--info mt-3 mb-0">
                    Você abriu esta NC para {nc.colaborador}. Aqui você acompanha o status e os dados que registrou.
                  </div>
                )}
              </div>
            </div>
                  ...
                  <div className="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
                    ...
                    {nc.status === "aberta" && (ehAdm || ehAutor || ehColaboradorDaNc) && (
                      <div className="d-flex align-items-center gap-2">
                        <Form.Control type="file" size="sm" className="sg-input" onChange={(e) => setArquivoNovo(e.target.files?.[0] || null)} />
                        <Botao variante="primario" tamanho="sm" carregando={enviandoArquivo} disabled={!arquivoNovo} onClick={enviarEvidencia}>Anexar</Botao>
                      </div>
                    )}
                  </div>

                  {erroEvidencias && <MensagemErro mensagem={erroEvidencias} onFechar={() => setErroEvidencias("")} />}
                  {carregandoEvidencias && <EstadoCarregamento mensagem="Carregando evidências..." compacto />}
                  {!carregandoEvidencias && evidencias.length === 0 && (
                    <EstadoVazio titulo="Nenhuma evidência anexada" descricao="As evidências enviadas aparecerão aqui." />
                  )}
                  {!carregandoEvidencias && evidencias.length > 0 && (
                      ...
                      {evidencias.map((ev) => (
                        <div key={ev.id} className="sg-evidencia-item">
                          <div className="me-3 min-w-0">
                            ...
                            {ev.url_temporaria && (ehImagem(ev.nome_original) ? (
                              <button type="button" className="sg-evidencia-item__link sg-evidencia-item__link--botao" onClick={() => setEvidenciaVisualizada(ev)}>Visualizar imagem</button>
                            ) : (
                              <a href={ev.url_temporaria} target="_blank" rel="noopener noreferrer" className="sg-evidencia-item__link">Abrir arquivo</a>
                            ))}
                          </div>
                          {(ehAdm || (ehAutor && nc.status === "aberta")) && (
                            <Botao variante="secundario" tamanho="sm" onClick={() => abrirModalExclusaoEvidencia(ev)}>Remover</Botao>
                          )}
                        </div>
                      ))}
                    </div>
      ...
      <Modal show={confirmarExclusaoNc} onHide={() => setConfirmarExclusaoNc(false)} centered>
        <Modal.Header closeButton><Modal.Title className="h5">Excluir NC #{id}?</Modal.Title></Modal.Header>
        <Modal.Body>Esta ação é permanente e não pode ser desfeita. Tem certeza?</Modal.Body>
        <Modal.Footer>
          <Botao variante="secundario" onClick={() => setConfirmarExclusaoNc(false)} disabled={excluindoNc}>Cancelar</Botao>
          <Botao variante="perigo" onClick={excluirNc} carregando={excluindoNc}>Sim, excluir</Botao>
        </Modal.Footer>
      </Modal>

      <Modal show={confirmarExclusaoEvidencia} onHide={() => { setConfirmarExclusaoEvidencia(false); setEvidenciaSelecionada(null); }} centered>
        <Modal.Header closeButton>
          <Modal.Title className="h5">Remover evidência{evidenciaSelecionada ? ` "${evidenciaSelecionada.nome_original}"` : ""}?</Modal.Title>
        </Modal.Header>
        <Modal.Body>Esta ação é permanente e remove o arquivo do storage e da lista de evidências desta NC. Deseja continuar?</Modal.Body>
        <Modal.Footer>
          <Botao variante="secundario" onClick={() => { setConfirmarExclusaoEvidencia(false); setEvidenciaSelecionada(null); }} disabled={excluindoEvidencia}>Cancelar</Botao>
          <Botao variante="perigo" onClick={confirmarExcluirEvidencia} carregando={excluindoEvidencia}>Sim, remover</Botao>
        </Modal.Footer>
      </Modal>
